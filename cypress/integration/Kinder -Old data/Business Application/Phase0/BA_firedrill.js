...
Cypress.on('uncaught:exception', (err, runnable) => {
  ...
  describe('Accident Report', () => {
    ...
    cy.wait(2000)
    cy.xpath('//a[contains(text(),"避難訓練")]').click({ multiple: true });
    cy.activeTabtitle('避難訓練');
        cy.wait(2000)
        cy.get('.custom-select').each(($li, index, $lis) => {
            ...
            cy.get('.custom-select').invoke('text').should('contain', '災害の種類')
        cy.wait(2000)
        })
        cy.get('.custom-select').each(($li, index, $lis) => {
            cy.get($li).should('have.css', 'font-family').and('match', /"Noto Sans JP", sans-serif/)
        ...
        cy.columnHeader();
        cy.wait(4000)
        cy.get('[role="columnheader"]').invoke('text').should('contain', '実施日')
        cy.get('[role="columnheader"]').invoke('text').should('contain', '災害の種類')
        cy.get('[role="columnheader"]').invoke('text').should('contain', '想定場所')
        cy.get('[role="columnheader"]').invoke('text').should('contain', '実施責任者')
    ...
    it('Batch process check', () => {
        ...
        cy.get('[type="button"]').should('have.css', 'font-size').and('eq', '14px');
        cy.get('[type="button"]').should('have.css', 'font-family').and('match', /"Noto Sans JP", sans-serif/);
        cy.get('div.pull-right.mb-2 > .batch-processing > #dropdown-form > #dropdown-form__BV_toggle_').click();
        cy.wait(2000)
        cy.checkBoxLabel();
        cy.wait(2000)
