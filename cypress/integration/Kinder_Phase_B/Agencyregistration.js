...
describe('K20 - Agency registration', () => {
  ...
  it('KIND-174 : Verification for Mandatory Field check', () => {
   cy.login();
    cy.wait(20000)
    cy.xpath(agencyRegistration).click({force: true})
  ...
  it('Phone Number Duplication', () => {
    cy.xpath(agencyListing).click({ force: true })
    cy.xpath(addButton).click({ force: true });
    cy.get(agencyName).type(data.agencyName1,{ force: true })
    cy.wait(8000)
    cy.get(phonenumber).type(data.phonenumberwhilecreating,{ force: true })
    cy.get(email).type(data.agencyEmailID1,{ force: true })
  ...
  it('Email Duplication', () => {
    ...
    cy.get(agencyName).type(data.agencyName1,{ force: true })
    cy.wait(8000)
    cy.get(phonenumber).type(data.phonenumberwhileediting,{ force: true })
    cy.get(email).type(data.agencyEmailID,{ force: true })
    cy.xpath(saveButton).click({ force: true });
