...
var allCheckbox = '#checkbox-group-allqq';
var saveButton = '//button[contains(text(),"保存")]';
//var dataFile = require('../../fixtures/testData.js')
...
it('KIND-300 - Verify validation with out role name',()=>{
 ...
 cy.login();
  ...
  cy.xpath(createButton).click({force:true});
...
it('Verify maximum validation of role name',()=>{
  ...
  cy.get(roleName).type(faker.lorem.paragraphs(),{force: true})
  cy.get(allCheckbox).click({force:true});
...
it('KIND-300 - Verify Creation of role',()=>{
  cy.wait(4000)
  cy.xpath(createButton).click();
  cy.get(roleName).clear({force: true})
  cy.get(roleName).type(data.role,{force: true})
  cy.get(allCheckbox).click({force:true});
  cy.xpath(saveButton).click();
  cy.wait(2000)
...
it('KIND-303 - Verify By Deleting the role ',()=>{
  cy.wait(5000)
  cy.xpath(createButton).click();
  cy.get(roleName).type(data.roleForDeletion,{force: true})
  cy.get(allCheckbox).click({force:true});
  cy.xpath(saveButton).click();
  cy.wait(2000)
  cy.get(groupList).within(()=>{
