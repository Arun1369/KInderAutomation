...
Cypress.on('uncaught:exception', (err, runnable) => {
  ...
  describe('Staff Registration', () => {
   ...
   it('KIND-BAPP-59 : Verification for Mandatory fields', () => {
      ...
      cy.wait(6000)
      cy.xpath(staffRegistration).click({ force: true })                                              
      cy.wait(6000)     
   ...
   it('Verify Staff Registration', () => {
    ...
    cy.wait(2000)
    //Click on the permission set up
    cy.xpath('//label[contains(text(),"全てチェック")]').click({multiple:true})
    cy.xpath(saveButton).click({ force: true })   
  ...
  it('Email Duplication', () => {
    ...
    cy.wait(3000)
    cy.xpath(cancelButton).click({ force: true })
  })


  it('Verify login with created staff', () => {
    cy.clearLocalStorage()
    cy.reload();
    cy.accessBALoginPage();
    cy.wait(8000)
    cy.get('[autocomplete="email"]').type(data.staffEmailID)
    cy.get('[autocomplete="password"]').type('admin123')
    cy.get('.btn').click();
    cy.wait(5000)
    cy.get('[autocomplete="email"]').should("not.exist")
